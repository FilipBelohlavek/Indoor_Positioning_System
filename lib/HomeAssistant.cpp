...
void HomeAssistant::connectToHA(PubSubClient CLIENT, char* nameX, char* nameY){
  ...
  x_doc["name"] = nameX;
  x_doc["stat_t"] = this->pub_topic_x;
  serializeJson(x_doc, x_configMessage);
  CLIENT.publish(this->config_topic_x, x_configMessage);

  y_doc["name"] = nameY;
  y_doc["stat_t"] = this->pub_topic_y;
  serializeJson(y_doc, y_configMessage);
  CLIENT.publish(this->config_topic_y, y_configMessage);  
}
...
void HomeAssistant::reconnect(PubSubClient CLIENT){
  while(!CLIENT.connected()){
    ...
    if(CLIENT.connect(clientId.c_str(), mqtt_user, mqtt_password)){
      ...
      CLIENT.subscribe(this->pub_topic_x);
      CLIENT.subscribe(this->pub_topic_y);
    }
    else{
...
void HomeAssistant::setConfigTopicX(char* sideX){
  strncpy(this->config_topic_x, "homeassistant/sensor/", sizeof(this->config_topic_x));
  strncat(this->config_topic_x, sideX, sizeof(this->config_topic_x));
  strncat(this->config_topic_x, "/config", sizeof(this->config_topic_x));
}

void HomeAssistant::setConfigTopicY(char* sideY){
  strncpy(this->config_topic_y, "homeassistant/sensor/", sizeof(this->config_topic_y));
  strncat(this->config_topic_y, sideY, sizeof(this->config_topic_y));
  strncat(this->config_topic_y, "/config", sizeof(this->config_topic_y));
}

char* HomeAssistant::getConfigTopicX(){
  return this->config_topic_x;
}

char* HomeAssistant::getConfigTopicY(){
  return this->config_topic_y;
}
