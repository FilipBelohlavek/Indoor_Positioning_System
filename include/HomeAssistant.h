...
#define INCOMING_PACKET_BUFFER_SIZE 50
  ...
  class HomeAssistant{
    private:
      char pub_topic_x[35];
      char pub_topic_y[35];
      char config_topic_x[60];
      char config_topic_y[60];
    ...
    public:
      ...
      //sets pub_topic_x to IPS/"nameX"
      void setPubTopicX(char* sideX);

      //sets pub_topic_y to IPS/"nameY"
      ...
      //sets config_topic_x to homeassistant/sensor/"nameX"/config
      void setConfigTopicX(char* sideX);

      //sets config_topic_y to "homeassistant/sensor/"nameY"/config"
      void setConfigTopicY(char* sideY);

      //returns config_topic_x
      char* getConfigTopicX();

      //returns config_topic_y
      char* getConfigTopicY();
